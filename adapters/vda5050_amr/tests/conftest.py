...
def _wait_for_broker_ready(port: int, timeout: float = 30.0) -> bool:
    ...
    _subscribe_and_wait (adapter.py) started reliably timing out on the
    very first subscribe of a module, immediately after this fixture's
    old bare-connect probe had just declared the broker ready. Proving
    a real subscribe+SUBACK round trip here, before any test's own
    client ever touches the broker, closes that gap instead of just
    giving the symptom a longer timeout to hide behind.
    """
